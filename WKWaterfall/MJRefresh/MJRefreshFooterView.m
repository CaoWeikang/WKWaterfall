...
//@property (assign, nonatomic) CGFloat lastCurrentPage;
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
         ...
         */
        // 失败文字设置
        self.failedText = @"点击加载更多";
        self.statusLabel.font = [UIFont systemFontOfSize:14.0f];
        self.statusLabel.textColor = [UIColor colorWithRed:153/255.0 green:153/255.0 blue:153/255.0 alpha:153/255.0];
        self.statusLabel.textAlignment = NSTextAlignmentCenter;
        
        // 添加手势
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(continueLoad:)];
...
- (void)continueLoad : (UITapGestureRecognizer *)sender
{
    if (!self.isRefreshing && sender.numberOfTouchesRequired == 1) {
        NSLog(@"开始加载");
...
- (UIActivityIndicatorView *)activityView
{
    if (!_activityView) {
        ...
        activityView.center = CGPointMake(self.mj_width * 0.5, self.mj_height * 0.4);
...
- (void)layoutSubviews
{
    ...
    // 修改父类位置坐标
    self.activityView.center = CGPointMake(self.mj_width * 0.5, self.mj_height * 0.4);
    // 修改箭头位置
    self.arrowImage.mj_x = -self.mj_width;
}
...
- (void)adjustStateWithContentOffset
{
    ...
    if (currentOffsetY + 60 <= happenOffsetY) return;
//    if (currentOffsetY < self.lastCurrentPage) return;
    
    // 快到看见尾部时开始刷新
    if (self.state == MJRefreshStateNormal && currentOffsetY + 60 >= happenOffsetY) {
           self.state = MJRefreshStateRefreshing;
    }
    // 失败时
    if (self.state == RefreshStateFailed && currentOffsetY >= happenOffsetY + self.mj_height + 50){
        self.state = MJRefreshStateRefreshing;
    }
...
- (void)setState:(MJRefreshState)state
{
	...
	switch (state)
    {
		case MJRefreshStateNormal:
        {
            ...
            } else if (RefreshStateFailed == oldState){
...
//            self.lastCurrentPage = self.scrollView.mj_contentOffsetY;
