...
//        animation.animationImages = @[[UIImage imageNamed:@"refresh1"]];
...
- (void)settingLabelText
{
	switch (self.state) {
        ...
        case RefreshStateFailed:
            // 添加失败文字
            self.statusLabel.text = self.failedText;
            break;
        default:
            break;
	}
...
- (void)setState:(MJRefreshState)state
{
    ...
    */
    if ( (self.state != MJRefreshStateRefreshing) && (self.state != RefreshStateFailed) ) {
        _scrollViewOriginalInset = self.scrollView.contentInset;
    }
